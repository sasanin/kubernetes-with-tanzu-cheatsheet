...
| Health check Kubectl CLI | Use `kubectl version --output=yaml` to see your client and server versions in yaml format. Make sure the CLI client can run on the latest supported Kubernetes version - check [here](https://kubernetes.io/releases/). || You will also be able to see the output in json format if changing to `--output=json`.|
