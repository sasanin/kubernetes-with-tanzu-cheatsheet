...
| Assign value to `GOOGLE_REGISTRY_PROJECT`| Open `tap-registry` and copy the path to it. Save it to something similar to `europe-north1-docker.pkg.dev/<YOUR_PROJECT>/tap-registry` for variable `GOOGLE_REGISTRY_PROJECT`. |  | |
